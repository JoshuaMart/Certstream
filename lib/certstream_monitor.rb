...
class CertstreamMonitor
  ...
  def preload_discovered_domains_cache
    ...
    if @db.respond_to?(:preload_discovered_domains_cache)
  ...
  def log_performance_stats
    ...
    if elapsed >= 60 # Log stats every minute
      current_queue_size = @queue.size rescue "unknown"
      ...
      if current_queue_size.is_a?(Integer) && current_queue_size > 10000
  ...
  def process_domain(domain)
    ...
    if @domain_cache[clean_domain] == :discovered || db.domain_already_discovered?(clean_domain)
      return
    end
